...
| `EXTRACTION_PROVIDER`                    | optional    | api      | `mock` (default), `anthropic`, or `openai`. Mock skips API calls. Real providers need the matching key.                                                                                                  |
| `ANTHROPIC_API_KEY`                      | conditional | api      | Required when `EXTRACTION_PROVIDER=anthropic`. Get one from anthropic.com.                                                                                                                               |
| `OPENAI_API_KEY`                         | conditional | api      | Required when `EXTRACTION_PROVIDER=openai`. Default model is `gpt-4o-2024-11-20` (vision). ADR-0015.                                                                                                     |
| `EXTRACTION_FALLBACK_PROVIDER`           | optional    | api      | A second provider (`anthropic`, `openai`, `mock`) different from the primary. On retryable infra failures of the primary, the worker tries this one once before failing the job. ADR-0015.               |
| `EXTRACTION_DAILY_BUDGET_USD`            | optional    | api      | Hard cap on AI spend per UTC day, system-wide (decimal USD). Unset = no system enforcement. `POST /extractions` returns 429 with `scope: "system"` when hit (ADR-0011).                                  |
| `EXTRACTION_DAILY_BUDGET_PER_USER_USD`   | optional    | api      | Hard cap on AI spend per UTC day, per user (decimal USD). Unset = no per-user enforcement. Stacks under the system cap; 429 carries `scope: "user"` when this is the one hit (ADR-0014).                 |
| `EXTRACTION_CONFIDENCE_REVIEW_THRESHOLD` | optional    | api      | Confidence cutoff between `succeeded` and `needs_review` (decimal 0..1). Defaults to 0.7. Raise to send more borderline extractions to manual review; lower to auto-confirm more aggressively. ADR-0005. |
...
- `@fastify/rate-limit` is global at 600 requests per minute per source IP. The FastifyAdapter trusts proxy headers so the IP comes from `X-Forwarded-For` when an LB sits in front. 600/min is comfortable for normal browsing and well above what `/healthz` and `/readyz` need in any realistic probe cadence, but it cuts off scripted enumeration before the public probe endpoints amplify into a DB roundtrip storm. Responses carry `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`; over-the-limit requests return 429 with `Retry-After`.
